...
Înainte de fiecare lucrare de revizie, autovehiculul trebuie asigurat împotriva oricăror mișcări nedorite: Poziționați schimbătorul de viteză în poziția \aW{neutră}, activați frâna de staționare și asigurați autovehiculul cu cale pentru blocarea roților.
\stopSymList
\stop
...
În cazul în care\index{Pericol+intoxicare} trebuie să porniți motorul într-un loc care nu dispune de o aerisire suficientă, lăsați-l în funcțiune doar timpul necesare\index{Pericol+gaze de eșapament}, pentru a evita intoxicațiile cu monoxid de carbon.
\stopPictPar
\startitemize
...
\PictPar
\textDescrHead{Pericol de accidente}
Toate\index{Modificări la autovehicul} modificările, pe care le efectuați la autovehicul îi pot afecta capacitatea de funcționare și siguranța operațională, \ProductId contribuind astfel la creșterea riscului de producere a unui accident sau unor leziuni.
\stopPictPar

\startPictPar
\PMwarranty
\PictPar
Pentru daunele produse\index{Condiții+de garanție} prin intervenții sau modificări proprii la \ProductId sau un agregat, \Boschung\ nu asigură nici un fel de garanție sau culanță.
\stopPictPar
\stoptextbackground
...
\PMgeneric
\PictPar
Utilizați doar lubrifianți, care sunt potriviți pentru părțile componente ale \ProductId. Din acest motiv, utilizați doar produse verificate și omologate de \Boschung\. Acestea sunt enumerate în lista de produse consumabile \atpage[sec:liqquantities]. Aditivii\index{Aditivi} pentru lubrifianți nu sunt necesari. În cazul în care adăugați aditivi, aceștia pot duce la anularea drepturilor de garanție\index{Condiții+garanție}. Pentru mai multe informații, adresați-vă Serviciului clienți \Boschung.
\stopPictPar
\stoptextbackground
...
\bTD cca. 50\,l\eTD
\eTR
\bTR \bTD Circuit hidraulic (opțiune \aW{Bio})\eTD
\bTD Ulei ATF \eTD
\bTD \liqC{dexron iii} \eTD
...
\bTD [nc=2] Apă și concentrat pentru spălarea parbrizului, \aW{S} vara, \aW{W} iarna; respectați raportul de amestec \eTD
...
Erorile care apar doar {\em sporadic} sunt afișate cu mențiunea \aW{SP}. Cauza erorilor sporadice poate fi \eG\ un contact slăbit sau o întrerupere de scurtă durată a conexiunii. În cazul în care apare în cursul a 50 de porniri o eroare sporadică nu mai apare, aceasta este ștearsă automat din eroare.

În cazul în care sunt identificate erori care afectează buna funcționare a motorului, pe ecranul Vpad se aprinde simbolul de control\aW{Diagnosticare motor} \textSymb{vpadWarningEngine1}.

Erorile stocate pot fi citite cu sistemul de diagnosticare a autovehiculului, precum și cu sistemul de măsurare și informare \aW{VAS 5051/B}.
...
Puteți utiliza și alte sisteme compatibile de diagnosticare, \eG\ \aW{DiagRA}.
\stopPictPar
\stoptextbackground
...
\startSteps [continue]
\item Apăsați pe Display, pe butonul \aW{Diagnosticare individuală a autovehiculului}.
\stopSteps
...
\startSteps [continue]
\item Apăsați pe Display, pe butonul \aW{01- Electronică motor}.
\stopSteps
...
\item Conectați VAS 5051/B an (a se vedea \in{sec.}[sSec:vw:diagTool:connect])
și selectați sistemul de comandă al motorului.
\item Selectați funcția de diagnosticare \aW{004-Conținut memorie stocare erori}.
\item Selectați funcția de diagnosticare \aW{004.01-Verificare conținut memorie stocare erori}.
\stopSteps
...
\item În cazul în care nu există nici o eroare în sistemul de comandă al motorului, pe display apare \aW{0 erori identificate}.
...
În cazul în care în memoria de date există una sau mai multe erori statice, vă rugăm să vă adresați serviciului clienți al firmei Boschung, pentru a remedia această eroare cu ajutorul \aW{Căutarea ghidată a erorii}.
...
\startSteps [continue]
\item Apăsați încă o dată tasta \aW{Mai departe/următorul} \inframed[strut=local]{>}, pentru a ajunge la planul de verificări.
\item Pentru a finaliza operațiunea de căutare asistată a erorii, apăsați tasta \aW{Skip/Salt} și apoi\aW{Finalizare}.
\stopSteps
...
\startSteps
\item Lăsați motorul să funcţioneze în gol.
\item Conectați VAS 5051/B (a se vedea \in{sec.}[sSec:vw:diagTool:connect]) și selectați sistemul de comandă al motorului.
\item Selectați funcția de diagnosticare \aW{004-Verificare conținut memorie}.
\item Selectați funcția de diagnosticare \aW{004.10-Ștergere conținut memorie}.
\stopSteps
...
\PMrtfm
\PictPar
Nivelul de ulei\index{Nivel+ulei de motor} nu trebuie în nici un caz să depășească marcajul \aW{Max.}. În caz contrar\index{Nivel umplere+ulei de motor} există riscul de producere de defecțiuni la catalizator.
\stopPictPar
\stoptextbackground
...
\item Nivel de umplere suficient; poate fi adăugat {\em ulei} până la atingerea marcajului \aW{A}.
\item Nivel de umplere insuficient; trebuie adăugat {\em ulei}, până când nivelul de umplere ajunge în zona \aW{B}.
\stopitemize
{\em În cazul unui nivel de umplere peste marcajul \aW{A} există riscul de producere de defecțiuni la nivelul catalizatorului.}
\stopfigtext
\stopSteps
...
Specificațiile necesare pentru ulei și cantitatea de umplere sunt prezentate în \in{sec.}[sec:liqquantities].
...
\item Umpleți cu ulei cu specificațiile corespunzătoare (a se vedea \in{sec.}[sec:liqquantities]).
\stopSteps
...
\item Schimbați\index{Filtru ulei+motor diesel} filtrele {\em înainte de} a realiza schimbul de ulei (a se vedea \in{sec.}[ssSec:vw:oilDraining]).
...
\item Pentru a evita umplerea în exces, după fiecare reumplere așteptați ca uleiul să curgă în baia de ulei, până la marcajul tijei de măsurare (a se vedea \in{sec.}[ssSec:vw:oilLevel]).
\stopitemize
\stopPictPar
\stoptextbackground
...
\item Efectuați un control pentru verificarea nivelului de umplere (a se vedea \in{sec.}[ssSec:vw:oilLevel]).
\stopSteps
...
\PMrtfm
\PictPar
Ridicați partea superioară a filtrului. În cazul în care este necesar, utilizați în acest sens o șurubelniță la canalul de montaj (\in{\LAa, Imaginea.}[fig:fuelfilter:detach]) și îndepărtați partea superioară a filtrului.
\stopPictPar
\stoptextbackground
...
\item Îndepărtați garnitura de etanșare (\in{\Ltwo, Imag.}[fig:fuelfilter:explosion]) din partea superioară a filtrului.
\item Curățați cu atenție partea inferioară și inferioară a filtrului.
\item Introduceți un element filtrant nou în partea inferioară a filtrului.
\item Umeziți o nouă garnitură (\in{\Ltwo, imag.}[fig:fuelfilter:explosion]) cu un pic de combustibil și aplicați-o în partea superioară a filtrului.
...
\item Verificați\index{Nivel umplere+agent refrigerant} nivelul de umplere cu agent refrigerant din vasul de expansiune: Trebuie să fie deasupra marcajului \aW{min}.
\stopSteps
...
Verificați\index{Densitatea antigelului} densitatea antigelului cu ajutorul unui refractometru adecvat (a se vedea \in{Imag.}[fig:refractometer]: VW T 10007).
Respectați scala 1: G12\hairspace ++ (a se vedea \in{Imag.}[fig:antifreeze]).
...
Filtrul pentru aer este accesibil prin ușa de revizie din spate, de pe partea dreaptă a autovehiculului (a se vedea \in{Imag.}[fig:airFilter]).
...
Mașini de măturat sunt frecvent utilizate în medii cu mari acumulări de praf. Din acest motiv verificarea și curățarea săptămânală a filtrelor de aer este deosebit de important. A se vedea și \about[table:scheduleweekly], \atpage[table:scheduleweekly]. Dacă este necesar, filtrul de aer trebuie înlocuit.


\subsubsubject{Autodiagnosticare}

Conducta de admisie dispune de un senzor de presiune (\Lfour, \in{Imag.}[fig:airFilter]), prin care pot fi stabilite pierderile de încărcare\footnote{Debit de aer redus ca urmare a permeabilității scăzute la aer a filtrului.} prin filtru.
Atunci când filtrul de aer este încărcat, pe displayul Vpad apare simbolul de control \textSymb{vpadWarningFilter} și mesajul de aer este \VpadEr{851} înregistrat.


\subsubsubject{Întreținere/înlocuire}

\startSteps
\item Trageți în jos eclisa \Lone de siguranță (\in{imag.}[fig:airFilter]).
\item Rotiți partea inferioară a carcasei în sensul invers al acelor de ceas \Ltwo și scoateți-o.
...
{\sla Instrumet de lucru special:}

Dorn de blocare \aW{VW T 10060 A} pentru fixarea elementului de menținere a tensiunii.

\startSteps
\item Marcați direcția de rulare a curelei de transmisie.
\item Balansați în sensul acelor de ceas brațul elementului de menținere a tensiunii, folosind o cheie inelară (\in {Imag.}[fig:belt:overview]).
\item Aliniați orificiile (a se vedea săgețile, \in {imag.}[fig:belt:tens]) și blocați elementul de menținere a tensiunii folosind dornul de blocare.
...
Kit de reparații, constând din instrucțiuni de reparații, curea de transmisie și instrument special de lucru.\footnote{A se vedea catalogul cu piese de schimb, de la \aW{Piese de întreținere}.}
...
\startSteps
\item Demontați curea de transmisie în modul descris (a se vedea \atpage[sSec:belt:change]).
\item Demontați componentele periferice (în funcție de echipare).
\item Deșurubați șurubul de siguranță (\in{\Ltwo, imag.}[fig:belt:tens:change]).
\stopSteps
...
\item Momentul de strângere: A se vedea \in{imag.}[fig:belt:tens:change].
\stopitemize
\stopPictPar
\stoptextbackground
...
Pentru măsurarea presiunii uleiului hidraulic, montați un manometru la una dintre \aW{conexiunile}de măsurare ale circuitului. Aveți grijă ca manometrul să prezinte un interval de măsurare corespunzător.
\stopSymList
...
Șurubul de golire se află în spatele rezervorului hidraulic, în fața roții anterioare din partea stângă (\in{Imag.}[fig:hydraulic:fluidDrain]).
...
\item Scoateți capacul filtrului de la rezervorul hidraulic (\in{Imag.}[fig:hydraulic:returnFilter]).
...
\startSteps
\item Demontați bobina ventilului electromagnetic (\in{\Lone, imag.}[graissage_bobine]).
\item Ungeți miezul (\in{\Ltwo, imag.}[graissage_bobine]) cu vaselina specială specificată și montați la loc bobina.
\stopSteps
...
Tamburii de frână \Lfour\ trebuie demontați la fiecare revizie regulată, mecanismul de frânare\Lseven\ trebuie curățat și garniturile \Lfive, \Lsix\ trebuie supuse unui control vizual (\in{imag.}[fig:brake:rear]).
...
Uzura anvelopelor trebuie verificată cu ajutorul indicatorilor de uzură, care se află într-o canelură a benzii de rulare (\in{imag.}[pneususure]).
...
\item Fisuri în suprafața de rulare: Anvelope prea vechi; cu timpul, cauciucul anvelopelor devine mai dur și mai fisurabil (\in{imag.}[pneusdomages]).
\stopitemize
...
\starttabulate[|l|r|p|]
\HL
\NC\md F 1 \NC 5 A \NC lămpi de stop, \aW{+\:15} OBD \NC\NR
\NC\md F 2 \NC 5 A \NC \aW{+\:15} comandă motor \NC\NR
\NC\md F 3 \NC 7,5 A \NC \aW{+\:30} comandă motor și OBD \NC\NR
\NC\md F 4 \NC 20 A \NC pompă carburant \NC\NR
\NC\md F 5 \NC 20 A \NC \aW{D\:+} alternator, \aW{+\:15} releu K 1 \NC\NR
...
\NC F 14 \NC 7,5 A \NC \aW{+\:15} Radio și cameră video \NC\NR
\NC F 15 \NC 10 A \NC \aW{+\:30} instalație de iluminat intermitent de semnalizare \NC\NR
\NC F 16 \NC 5 A \NC Lumină coloană de direcție \NC\NR
\NC F 17 \NC 7,5 A \NC \aW{+\:30} Radio, întrerupător lumină și sistem de iluminat intern \NC\NR
\NC F 18 \NC — \NC {[}liber{]} \NC\NR
\NC F 19 \NC 20 A \NC \aW{+\:30} RC 12 față \NC\NR
\NC F 20 \NC 20 A \NC \aW{+\:30} RC 12 spate \NC\NR
...
\NC F 25 \NC 2 A \NC \aW{+\:15} RC 12 față \NC\NR
\NC F 26 \NC 2 A \NC \aW{+\:15} RC 12 spate \NC\NR
...
\NC F 31 \NC 5 A \NC \aW{+\:15} Consolă multifuncțională/Vpad \NC\NR
...
În cazul în care în timpul unei verificări se constată că reglarea trebuie modificată, slăbiți șurubul de siguranță și corectați înclinația astfel încât aceasta să corespundă prevederilor legale (a se vedea \in{imag.}[fig:lighting:adjustment]). Strângeți la loc șurubul de siguranță.
...
\item Activați\index{Aspirarea} modul \aW{de lucru} (apăsați butonul din partea exterioară manetei de selectare a vitezelor de mers).
